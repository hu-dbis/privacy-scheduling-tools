...
class PupTwct:
    def __init__(self, schedule: Schedule, privacy_threshold: float, utility_threshold: float):
        ...
        self.original_utility = single_machine_utilities.calculate_twct(self.original_schedule)
        self.result = {}
    ...
    def check_pup(self, schedule: Schedule) -> bool:
        ...
        if utility_satisfied:
            if self.privacy_threshold == 0:
                necessary_solution_size = 0
            else:
                necessary_solution_size = 1 / self.privacy_threshold
            result_privacy = self.adversary.execute_attack(schedule, necessary_solution_size)
            if result_privacy['solution_count'] == necessary_solution_size or result_privacy['original_weight_vector_seen'] is False:
